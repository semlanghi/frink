...
if [ -z "$CONFLUENT_HOME" ]
then
      CONFLUENT_HOME="/Users/samuelelanghi/Documents/platforms/confluent-6.2.0"
else
...
java -Xmx2g -cp $PROJECT_DIR/target/frink-1.0-SNAPSHOT.jar linearroad.ooo.DataGenerator data/ooo_final_config.json
...
$CONFLUENT_HOME/bin/kafka-topics --delete --bootstrap-server localhost:9092 --topic "$topic" --if-exists
$CONFLUENT_HOME/bin/kafka-topics --create --bootstrap-server localhost:9092 --replication-factor 1 --partitions 1 --topic "$topic"
wait
