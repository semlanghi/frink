...
sourceFile=${input}
echo "sourceFile is $sourceFile and input is $input"
...
$CONFLUENT_HOME/bin/kafka-topics --delete --bootstrap-server ${bootstrap} --topic "$topic" --if-exists
$CONFLUENT_HOME/bin/kafka-topics --create --bootstrap-server ${bootstrap} --replication-factor 1 --partitions 1 --topic "$topic"
wait

echo "Start producing linear road data to Topic $topic from file $sourceFile:"
echo java -Xmx2g -cp ../target/frink-1.0-SNAPSHOT.jar linearroad.datagenerator.LinearRoadKafkaDataProducer ${bootstrap} ${topic} ${sourceFile} ${maxevents} #&> loading.out
java -Xmx2g -cp ../target/frink-1.0-SNAPSHOT.jar linearroad.datagenerator.LinearRoadKafkaDataProducer ${bootstrap} ${topic} ${sourceFile} ${maxevents} #&> loading.out
