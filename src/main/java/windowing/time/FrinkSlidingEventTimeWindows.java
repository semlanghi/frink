 ...
 */
@PublicEvolving
public class FrinkSlidingEventTimeWindows<I> extends WindowAssigner<I, TimeWindow> implements SingleBufferWindowing<I, GlobalWindow> {
    ...
    public FrinkSlidingEventTimeWindows(long size, long slide) {
        this(size, slide, 0);
    }

    public FrinkSlidingEventTimeWindows(long size, long slide, long offset) {
     ...
     */
    public static FrinkSlidingEventTimeWindows of(Time size, Time slide) {
        return new FrinkSlidingEventTimeWindows(size.toMilliseconds(), slide.toMilliseconds(), 0);
    }
     ...
     * @return The time policy.
     */
    public static FrinkSlidingEventTimeWindows of(Time size, Time slide, Time offset) {
        return new FrinkSlidingEventTimeWindows(size.toMilliseconds(), slide.toMilliseconds(), offset.toMilliseconds());
    }
            ...
            public void evictAfter(
                ...
                try {
                    for (Iterator<TimestampedValue<I>> itTsValues = elements.iterator(); itTsValues.hasNext(); ) {
                        ...
                        long currentWatermark = evictorContext.getCurrentWatermark();
                        if (next.getTimestamp() + allowedLateness * 1000L <= currentWatermark)
                            itTsValues.remove();
    ...
    public Trigger<I, GlobalWindow> singleBufferTrigger() {
        return new CustomTrigger<I, TimeWindow>(this);
    }
}
